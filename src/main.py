...
def onEnterpress():
    command_name = text_box.get()  # Get the text from the Entry widget
    command_path=text_box2.get()
    if(voice_commands.add_command(command_name,command_path)==False):
        onEnterClick()
...
def onEnterClick():
    ...
    if not os.path.exists(command_path):
      ...
      n=4
      update_images()
      voice_commands.add_command(command_name,command_path)
...
def update_images():
    ...
    global SavedScreenResized
    original_image_resized = home_image_one.resize((screen_width-280, screen_height-120))
    second_image_resized = home_image_two.resize((screen_width-280, screen_height-120))
    settings_image_resized = setting_image.resize((screen_width-280, screen_height-120))
    add_CommandScreen_resized = addCommandScreen.resize((screen_width-280, screen_height-120))
    ErrorImageResized=errorScreen.resize((screen_width-280, screen_height-120))   
    SavedScreenResized=savedScreen.resize((screen_width-280, screen_height-120))
    if n == 0:
        ...
        text_box2.place_forget()
        enter.place_forget()
        current_image=SavedScreenResized
        cross.place_forget()
        back.place(x=45,y=485)
        set_Image()
...
if __name__ == "__main__":
    ...
    savedScreen=Image.open("SAVED.png")
    ...
    text_box.insert(tk.END, temp)
  
    text_box.place_forget()
    '''text_box.bind("<Return>", onEnterPress())
 '''
    temp="Enter Path"
